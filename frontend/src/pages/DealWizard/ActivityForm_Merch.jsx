...
const ActivityForm_Merch = ({ nextStepUrl }) => {
  ...
  useEffect(() => {
    if (deal?.obligations?.['Merch and Products'] && Object.keys(deal.obligations['Merch and Products']).length > 0) {
      const data = deal.obligations['Merch and Products'];
      reset({
        ...data,
  ...
  const onContinue = async (formData) => {
    const newObligations = { ...deal.obligations, "Merch and Products": formData };
  ...
  return (
    <DealWizardLayout
      ...
      instructions="Provide the details for the merchandise being sold."
      ...
      <VStack as="form" spacing={4} onSubmit={handleSubmit(onContinue)}>
        <Controller name="description" control={control} render={({ field }) => (
          <FormControl isInvalid={errors.description}>
            <FormLabel>Description of items being sold</FormLabel>
            <Textarea {...field} />
